  ...
  if ((r && typeof r == "object") || typeof r == "function")
    for (let n of De(r))
      !Pe.call(i, n) &&
        ...
        de(i, n, {
          get: () => r[n],
          enumerable: !(e = Oe(r, n)) || e.enumerable,
        });
  return i;
};
var ce = (i, r, t) => (
        ...
        let r = "The message port closed before a response was received.",
          t = (e) => {
            ...
            let u = (m) =>
              ...
              o = (m, c) =>
                function (_, ...R) {
                  if (R.length < c.minArgs)
                    throw new Error(
                      `Expected at least ${c.minArgs} ${f(c.minArgs)} for ${m}(), got ${R.length}`,
                    );
                  if (R.length > c.maxArgs)
                    throw new Error(
                      `Expected at most ${c.maxArgs} ${f(c.maxArgs)} for ${m}(), got ${R.length}`,
                    );
                  return new Promise((D, P) => {
                    if (c.fallbackToNoCallback)
                      try {
                        _[m](...R, l({ resolve: D, reject: P }, c));
                      } catch (y) {
                        (console.warn(
...
function Ae(i) {
  for (let r of Ge) if (r.mime_reg.test(i)) return I(r.demuxer);
  return b;
}
function He(i, r) {
...
async function Se(i, r, t, e = !1, n = !1) {
  let a;
  try {
    let o = new W();
  ...
  for (let o of u) {
    ...
    if (
      ...
      (e
        ...
        : n && (T.search = ""),
        f.push({ demuxer: h, quality: s, av: { video: C, audio: T } }));
    } else f.push({ demuxer: h, quality: s, av: { video: C, audio: !1 } });
  }
  return ((f = _e(f)), be(f) ? M(f) : w("Empty playlist"));
}
async function we(i) {
